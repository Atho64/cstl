// @module ai-check.js — AI Check tab: copy, parse, render, apply corrections

import { state, ui } from './state.js';
import { isTranslated } from './state.js';
import { unescapeStoredNewlines, escapeStoredNewlines } from './string-utils.js';
import { getLineDisplayName, formatLineLabel } from './luca-engine.js';
import { rebuildDisplayState, renderPreviewRows, syncCheckboxUI, updateButtonStates, pushUndoSnapshot, refreshAll, flashHint } from './render.js';
import { queueAutoSave } from './project.js';
import { applyPromptVariables } from './ai-format.js';
import { getGlossaryPrompt } from './glossary.js';
import { DEFAULT_AI_CHECK_PROMPT } from './constants.js';


export function getSelectedTranslatedLines() {
  return state.lines.filter(l => state.selectedLines.has(l.line_num) && isTranslated(l));
}

export function getLineForAiCheck(line) {
...
export function setAiCheckStatus(message, keepAlive = false) {
...
export async function onCopyForAiCheck() {
...
export function parseAiCheckBlocks(text) {
...
export function onParseAiCheck() {
  try {
...
export function renderAiCheckCorrections() {
...
export function stripDuplicateSpeakerPrefix(text, name) {
...
export function onApplyAiCheckCorrections() {
...
export function onClearAiCheck() {
  state.aiCheckCorrections = [];
