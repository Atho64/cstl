// @module ai-format.js — AI translation format helpers: export, parse, detect
...
export function applyPromptVariables(prompt) {
  if (!prompt) return "";
  return prompt
...
export function normalizeAiTranslationFormat(value) {
...
export function getDefaultPromptHeaderForFormat(format) {
...
export function formatLineForAiExport(line) {
...
export function formatLineForAiExportXml(line) {
...
export function getSelectedTranslationText(includeTranslated = true) {
...
export function getSelectedTranslationPlainText(includeTranslated = true) {
  const sel = state.lines.filter(l => state.selectedLines.has(l.line_num) && (includeTranslated || !isTranslated(l)));
  return sel.map(l => {
    const dN = l.name || "";
...
export function buildSelectedTranslationExport(includeTranslated = true) {
...
export function getTranslationPastePlaceholder() {
  const fmt = normalizeAiTranslationFormat(state.aiTranslationFormat);
...
export function detectTranslationPasteFormat(text) {
...
export function parseTranslationXml(text) {
...
export function parseTranslationBlocks(text) {
...
export function parseTranslationNumberedPaste(text) {
