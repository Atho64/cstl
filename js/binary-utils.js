// @module binary-utils.js — Binary encoding and byte manipulation helpers

// TODO: import from project.js when available
// Temporary inline fallback until project.js is created
...
export function latin1BytesToString(bytes) {
  let s = "";
...
export function splitBufferToLines(buffer) {
  const lines = [];
  let start = 0;
...
export function joinLinesToBuffer(lineArrays) {
...
export async function readEpubSourceForBackup(epubSourceId) {
...
export async function writeEpubSourceFromBackup(epubSource) {
...
export async function cloneExistingEpubSource(epubSourceId) {
  const root = await getOpfsRoot();
